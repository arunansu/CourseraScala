...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    def outputMax(i: Int, mx: Float): Unit = if (i < input.length) {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def maxAngle(i: Int, mx: Float): Float = {
      if (i < until) {
        val angle = max(input(i) / i, mx)
        maxAngle(i + 1, angle)
      }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    val sAngle = Math.max(input(from) / from, startingAngle)
    output(from) = sAngle
    downsweepSequential(input, output, sAngle, from + 1, until)
  }
