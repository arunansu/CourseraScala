...
object ParallelParenthesesBalancing {
  ...
  def balance(chars: Array[Char]): Boolean = {
    def checkOpenParentheses(chars: Array[Char], openCount: Int): Boolean = {
      if (chars.isEmpty) openCount == 0
      else if (openCount < 0) false
      else if (chars.head == '(') checkOpenParentheses(chars.tail, openCount + 1)
      else if (chars.head == ')') checkOpenParentheses(chars.tail, openCount - 1)
      else checkOpenParentheses(chars.tail, openCount)
    }
    checkOpenParentheses(chars, 0)
  }
